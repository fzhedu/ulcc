...
#define TDIFF(t1, t2)                                   \
  (((t2).tv_sec + ((double)((t2).tv_usec)) / 1000000) - \
...
void *thread_test(void *param) {
  ...
  if (it % 2) {
    ...
    for (i = 0; i < 3000; i++) {
      for (p = data1_start; p < data1_end; p += stride) {
        if ('x' == *p) ++num;
      }
    }
    gettimeofday(&t2, NULL);
    printf("There are %d x in strong space\n", num);
    printf("%s ULCC support: strong LLC locality - %.4lf s\n",
           ulcc_enabled ? "With" : "Without", TDIFF(t1, t2));
  } else {
    gettimeofday(&t1, NULL);
    for (i = 0; i < 700; i++) {
      for (p = data2_start; p < data2_end; p += stride) {
        if ('x' == *p) ++num;
      }
    }
    gettimeofday(&t2, NULL);
    printf("There are %d x in weak space\n", num);
...
void *thread_test_simd(void *param) {
  long it = (long)param;
  int i, num = 0, stride = 32;
  char *p, r;
  struct timeval t1, t2;
  unsigned int res;
  // 8 bytes, 8*8 bits
  __m256i zero256 = _mm256_set1_epi64x('x'), data, cmp;
  printf("starting ------------------%ld\n", it);
  if (it % 2) {
    usleep(500);
    gettimeofday(&t1, NULL);
    for (i = 0; i < 3000; i++) {
      for (p = data1_start; p < data1_end; p += stride) {
        data = _mm256_load_si256((__m256i *)p);
        cmp = _mm256_cmpeq_epi64(data, zero256);
        res = _mm256_movemask_epi8(cmp);
        // res = _pext_u32(res, 0x80808080);
        res = (_mm_popcnt_u32(res) >> 3);
        num += res;
      }
    }
    gettimeofday(&t2, NULL);
...
int test() {
  ...
  /* Data initialization */
  for (p = data1_start; p < data1_end; p += ULCC_PAGE_BYTES) *p = 'x';
  for (p = data2_start; p < data2_end; p += ULCC_PAGE_BYTES) *p = 'x';

  printf(
      "Start testing: improving the performance of a strong-locality loop"
  ...
  puts(">>>>>>>>>>>>>>>>>>simd<<<<<<<<<<<<<<<<<<<<");
  for (i = 0; i < 2; i++)
    if (pthread_create(&tid[i], NULL, thread_test_simd, (void *)i)) {
      printf("failed to create thread %d!\n", i);
      exit(-1);
    }
  for (i = 0; i < 2; i++) pthread_join(tid[i], NULL);
  ...
  if (cc_remap((unsigned long *)&data1_start, (unsigned long *)&data1_end, 1,
               &regn, CC_ALLOC_MOVE | CC_MAPORDER_SEQ) < 0) {
